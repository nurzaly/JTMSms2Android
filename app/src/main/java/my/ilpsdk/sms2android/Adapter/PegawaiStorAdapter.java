...
import my.ilpsdk.sms2android.Adapter.ViewHolder.PegawaiStorViewHolder;
 ...
 * Date: 24/05/15
 */
public class PegawaiStorAdapter extends RecyclerView.Adapter<PegawaiStorViewHolder> {

    private final LayoutInflater mInflater;
    private final List<KeluaranModel> mModels;

    public PegawaiStorAdapter(Context context, List<KeluaranModel> models) {
        mInflater = LayoutInflater.from(context);
        mModels = new ArrayList<>(models);
    }

    @Override
    public PegawaiStorViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        final View itemView = mInflater.from(parent.getContext()).inflate(R.layout.pegawaistor_list, parent, false);
        return new PegawaiStorViewHolder(itemView);
    }

    @Override
    public void onBindViewHolder(PegawaiStorViewHolder holder, int position) {
